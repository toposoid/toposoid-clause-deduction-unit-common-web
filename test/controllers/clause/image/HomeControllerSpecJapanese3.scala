...
class HomeControllerSpecJapanese3 extends PlaySpec with BeforeAndAfter with BeforeAndAfterAll with GuiceOneAppPerSuite with DefaultAwaitTimeout with Injecting {
  ...
  "The specification21" should {
    "returns an appropriate response" in {
      ...
      registerSingleClaimForFeature(KnowledgeForParser(propositionId2, sentenceId2, knowledge3), transversalState)
  ...
  "The specification23A" should {
    "returns an appropriate response" in {
      ...
      registerSingleClaimForFeature(KnowledgeForParser(propositionId3, sentenceId3, knowledge3), transversalState)
  ...
  "The specification27" should {
    "returns an appropriate response" in {
      ...
      val updatedAsosJson = addImageInfoToAnalyzedSentenceObjects(lang=lang, inputSentence, List(getImageInfo(referenceParaA, imageBoxInfoParaA, transversalState), getImageInfo(referenceParaB, imageBoxInfoParaB, transversalState), getImageInfo(referenceParaC, imageBoxInfoParaC, transversalState), getImageInfo(referenceParaD, imageBoxInfoParaD, transversalState)), transversalState)
      //val updatedAsosJson = TestUtils.analyzeByBaseDeductionUnit(json, transversalState)
      val fr = FakeRequest(POST, "/execute")
        .withHeaders("Content-type" -> "application/json", TRANSVERSAL_STATE.str -> transversalStateJson)
        .withJsonBody(Json.parse(updatedAsosJson))
  ...
  "The specification30" should {
    "returns an appropriate response" in {
      ...
      registerSingleClaimForFeature(KnowledgeForParser(propositionId1, sentenceId1, knowledge4), transversalState)
